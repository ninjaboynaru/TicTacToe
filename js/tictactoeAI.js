...
var AI = {
	...
	EvaluateBoard: function(pureBoard, aiCharacter)
	{
		...
		{
			if(pureBoard[row][0] == pureBoard[row][1] && pureBoard[row][1] == pureBoard[row][2] && pureBoard[row][2] != false)
			{
		...
		{
			if(pureBoard[0][col] == pureBoard[1][col] && pureBoard[1][col] == pureBoard[2][col] && pureBoard[2][col] != false)
			{
		...
		if(pureBoard[0][0] == pureBoard[1][1] && pureBoard[1][1] == pureBoard[2][2] && pureBoard[2][2] != false)
		{
			if(pureBoard[1][1] == aiCharacter){return 1;}
			else{return -1;}
		}
		if(pureBoard[2][0] == pureBoard[1][1] && pureBoard[1][1] == pureBoard[0][2] && pureBoard[0][2] != false)
		{
			if(pureBoard[1][1] == aiCharacter){return 1;}
			else{return -1;}
		}
		...
		for(var y = 0; y < 3; y++)
		{
			for(var x = 0; x < 3; x++)
			{
				if(pureBoard[y][x] == "" || pureBoard[y][x] == " ")
				{
					var d = false;
					if(x == 2 && y == 0){d = true;}
					currentMoveValue = this.MoveValue(x,y, pureBoard, aiCharacter, playerCharacter, 0, true, d);
